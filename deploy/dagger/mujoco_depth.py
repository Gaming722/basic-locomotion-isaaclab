...
VFOV 58 deg, mounted at (0.26, 0, 0.12) in the trunk frame — matching the
IsaacLab Tiled/RayCaster students), so this class is a thin wrapper around
``mujoco.Renderer``. Raw metric depth is returned; the full training
preprocessing (clip [0.1, 2.0] + blur + history/delay) is applied in
...
class MujocoDepthCamera:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, camera_id: int, *, width: int = 240, height: int = 140):
        self.model = model
        self.data = data
